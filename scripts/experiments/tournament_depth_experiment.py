...
# Avoid matplotlib/font cache overhead in headless benchmark runs.
...
from SimPyTest.policies import POLICIES, set_tournament_depth, clear_tournament_cache
...
def main():
    ...
    for spec in scenarios:
        ...
        print(
            f"Running {len(tasks)} jobs in parallel with {max_workers} worker process(es) " f"(set TDE_MAX_WORKERS to override)",
            flush=True,
        )
            ...
            with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
                ...
                for future in concurrent.futures.as_completed(futures):
                    ...
                    try:
                        ...
                        print(
                            f"Task failed: kind={task['kind']} scenario={task['scenario']} " f"policy={task['policy_name']} depth={task['depth']} seed={task['seed']}",
                            flush=True,
                        )
                        raise
