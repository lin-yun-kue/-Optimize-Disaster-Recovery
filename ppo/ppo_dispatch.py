...
VALIDATION_SEEDS = list(range(2 * 10**5, 2 * 10**5 + 10))
...
class DeepSetsDispatchPolicy(MaskableMultiInputActorCriticPolicy):
    def __init__(
        self,
        ...
        deepset_hidden_dim: int = 128,
        deepset_depth: int = 2,
        deepset_dropout: float = 0.1,
    ):
        self.deepset_hidden_dim = deepset_hidden_dim
        self.deepset_depth = deepset_depth
        self.deepset_dropout = deepset_dropout
        super().__init__(
            observation_space,
            action_space,
            lr_schedule,
    ...
    def _dispatch_config(self) -> DispatchScoringConfig:
        return DispatchScoringConfig(
            current_dim=CURRENT_RESOURCE_FEATURES,
            global_dim=GLOBAL_STATE_FEATURES,
            candidate_dim=DISASTER_FEATURES,
            output_dim=int(cast(spaces.Discrete[Any], self.action_space).n),
            hidden_dim=self.deepset_hidden_dim,
    ...
    def _coerce_action_mask(
        ...
        if action_masks is None:
            mask = obs["valid_actions"].to(dtype=torch.float32)
        elif isinstance(action_masks, torch.Tensor):
            mask = action_masks.to(device=obs["current_resource"].device, dtype=torch.float32)
        else:
            mask = torch.as_tensor(action_masks, device=obs["current_resource"].device, dtype=torch.float32)
        if mask.ndim == 1:
            mask = mask.unsqueeze(0)
        if mask.shape[0] != obs["current_resource"].shape[0]:
            mask = mask.expand(obs["current_resource"].shape[0], -1)
        return mask
    ...
    def _obs_parts(
        ...
        if not isinstance(obs, dict):
            raise TypeError("DeepSetsDispatchPolicy expects dict observations")
        current_resource = obs["current_resource"].to(dtype=torch.float32)
        global_state = obs["global_state"].to(dtype=torch.float32)
        candidate_disasters = obs["candidate_disasters"].to(dtype=torch.float32)
        valid_actions = self._coerce_action_mask(obs, action_masks)
    ...
    def _distribution_from_obs(
        ...
        obs: PyTorchObs,
        action_masks: np.ndarray | torch.Tensor | None = None,
    ) -> tuple[MaskableDistribution, torch.Tensor]:
        current_resource, global_state, candidate_disasters, valid_actions = self._obs_parts(obs, action_masks)
        logits = self.actor(current_resource, global_state, candidate_disasters, valid_actions)
        distribution = self.action_dist.proba_distribution(action_logits=logits)
        distribution.apply_masking(valid_actions > 0)
        return distribution, valid_actions

    @override
    def _build(self, lr_schedule: Schedule) -> None:
        dispatch_config = self._dispatch_config()
        self.actor = DispatchScoringModel(dispatch_config)
        self.critic = DispatchValueModel(dispatch_config)
        ...
        optimizer_class = cast(Any, self.optimizer_class)
    ...
    def forward(
        ...
        distribution, valid_actions = self._distribution_from_obs(obs, action_masks)
        current_resource, global_state, candidate_disasters, _ = self._obs_parts(obs, valid_actions)
        values = self.critic(current_resource, global_state, candidate_disasters, valid_actions)
    ...
    def get_distribution(
        ...
        distribution, _valid_actions = self._distribution_from_obs(obs, action_masks)
        return distribution
    ...
    def predict_values(self, obs: PyTorchObs) -> torch.Tensor:
        current_resource, global_state, candidate_disasters, valid_actions = self._obs_parts(obs)
    ...
    def _get_constructor_parameters(self) -> dict[str, Any]:
        ...
        data.update(
            dict(
                deepset_hidden_dim=self.deepset_hidden_dim,
                deepset_depth=self.deepset_depth,
                deepset_dropout=self.deepset_dropout,
            )
        )
        return data
...
class SeedCyclingEnv(DisasterResponseGym):
    ...
    def __init__(self, training_seeds: list[int], controller_name: str, stage: CurriculumStage):
        ...
        self.seed_index = 0
        super().__init__(max_visible_disasters=MAX_VISIBLE_DISASTERS, sorting_strategy=SORTING_STRATEGY, scenario_config=stage.scenario_config, controller_name=controller_name)
