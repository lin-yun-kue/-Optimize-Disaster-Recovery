...
def plot_results(results: list[dict[str, Any]], sweep_dir: Path) -> Path:
    ...
    for axis, (metric_key, metric_label) in zip(axes, metric_specs):
        for freeze_updates in freeze_updates_values:
            filtered = [item for item in results if item["freeze_actor_updates"] == freeze_updates]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Sweep train_critic hyperparameters (total_timesteps, freeze_actor_updates).")
    ...
    for timesteps in timesteps_list:
        for freeze_updates in freeze_list:
            ...
            config = PPOConfig(total_timesteps=timesteps, freeze_actor_updates=freeze_updates, seed=args.seed, device="cuda", save_dir=str(run_save_dir), actor_checkpoint=DEFAULT_ACTOR_CHECKPOINT)
