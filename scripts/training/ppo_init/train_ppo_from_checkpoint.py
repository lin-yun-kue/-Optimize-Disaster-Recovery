...
class PPOConfig:
    ...
    save_dir: str = "experiment_results/ppo_init_actor_from_checkpoint"
    checkpoint_path: str = "experiment/ppo_custom"
...
def resolve_checkpoint(path_str: str) -> Path:
    ...
    if candidate.is_dir():
        direct_ckpt = candidate / "checkpoint.pt"
        if direct_ckpt.exists():
            return direct_ckpt

        run_checkpoints = sorted(candidate.glob("*/checkpoint.pt"), key=lambda p: p.stat().st_mtime)
        if run_checkpoints:
            return run_checkpoints[-1]

    raise FileNotFoundError(f"Cannot locate checkpoint.pt from '{path_str}'.")


def load_actor_critic_checkpoint(agent: PPOAgent, checkpoint_path: str, device: torch.device) -> dict[str, Any]:
    ...
    return {
        ...
        "checkpoint_generated_at_utc": payload.get("generated_at_utc"),
...
def evaluate_agent(agent: PPOAgent, config: PPOConfig, device: torch.device, deterministic: bool = True) -> dict[str, Any]:
    ...
    for seed in config.checkpoint_seeds:
        env = make_env(config, controller_name="ppo_actor_checkpoint")
...
def train(config: PPOConfig) -> Path:
    ...
    env = make_env(config, controller_name="ppo_actor_checkpoint_init")
    obs, _ = env.reset(seed=config.seed)

    agent = PPOAgent(config).to(device)

    checkpoint_metadata = load_actor_critic_checkpoint(agent, config.checkpoint_path, device)
    agent.critic.eval()
    ...
    while timestep < config.total_timesteps:
        ...
        losses = ppo_actor_update(agent, optimizer, batch, config)
        update_idx += 1
    ...
    print(f"Saved actor+critic-init PPO artifacts to {run_dir}")
    print(f"Training wall time: {total_wall_time_s:.2f}s ({format_duration(total_wall_time_s)})")
    return run_dir


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train actor with both actor/critic initialized from a previous PPO checkpoint while keeping critic fixed.")
    ...
    parser.add_argument(
        "--checkpoint-path",
        type=str,
        default=PPOConfig.checkpoint_path,
        help="Path to checkpoint.pt, a run directory, or a directory containing run folders.",
    )
